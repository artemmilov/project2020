...
bool body::init(float X, float Y, float WIDTH, float HEIGHT, float VX, float VY, bool GENERAL_STICKY)
{
	x = X;
	y = Y;
	if (WIDTH < 0 || HEIGHT < 0)
...
void body::update(float cur_time)
{
	...
	x += v_x * cur_time;
	y += v_y * cur_time;
	if ((stick_clock.getElapsedTime().asMilliseconds() >= 20) && general_sticky)
		sticky = true;
